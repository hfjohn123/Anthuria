...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  return (
    <aside
      ref={sidebar}
      className={`fixed left-0 top-17.5 z-999 flex h-[calc(100vh-3rem)]  flex-col overflow-clip bg-black duration-300 ease-linear dark:bg-boxdark lg:h-screen lg:top-0 lg:translate-x-0 ${
    ...
    >
      ...
      <nav
        className={clsx(
          'mt-5 pt-4 lg:mt-6 grow flex flex-col justify-between h-[calc(100%-5.5rem)]) overflow-y-auto no-scrollbar',
          (expanded || sidebarOpen) && ' px-4 lg:px-6',
        )}
      >
        {/* <!-- Menu Group --> */}
        <div className="shrink">
          ...
          <ul className=" mb-6 flex shrink flex-col gap-1.5">
            {/* <!-- Menu Item Dashboard --> */}
            ...
            {user_applications_locations.some(
              (location) => location.id === 'trigger_words',
            ) && (
              <li>
                <Link
