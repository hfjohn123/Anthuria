...
function SignIn() {
  ...
  const {isPending: isSessionPending, data: isSession, error: sessionError, isError: isSessionError} = useQuery({
    queryKey: ['session', route],
    queryFn: async () => await Session.doesSessionExist(),
  })
  const {isPending, data: canny_jwt, error, isError} = useQuery({
  ...
  if (isError) {
    createToast('Failed to Fetch Canny Token', error.message, 3, 'Login Failed');
  }
    ...
    (async () => {
      if (!isSessionPending && isSession) {
        ...
        if (companyId && redirect && canny_jwt) {
          window.location.assign(`https://canny.io/api/redirects/sso?companyID=${companyId}&ssoToken=${canny_jwt}&redirect=${redirect}`);
  ...
  return (
    <NoBar>
      <div
        className = "rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark mt-10">
