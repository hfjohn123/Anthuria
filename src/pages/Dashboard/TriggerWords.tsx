...
export default function TriggerWords() {
  ...
  const handleFilterChange = (selected: MultiValue<{
    ...
    if (selected.length === 0) {
      setTableState(
        (prev) => ({...prev, columnFilters: prev.columnFilters.filter((f) => f.id !== name)}),
      )
      return;
    }
    setTableState((prev) => ({
      ...prev,
      columnFilters: prev.columnFilters.filter((f) => f.id !== name).concat({
        id: name || '',
      ...
      <div className = "grid grid-cols-12 ">
        <div className = " mt-5 col-span-12 bg-white dark:bg-boxdark shadow-default ">
          <div className = "flex items-center border-b border-stroke">
            ...
            <input
              onChange = {(e) => {
                setTableState((prev) => ({...prev, globalFilter: e.target.value}))
              }}
            ...
            <ReactSelectButton options = {table._getColumnDefs().map((c) => ({value: c.accessorKey, label: c.header}))}
                               ...
                               value = {Object.entries(tableState.columnVisibility).filter(([, v]) => v).map(([k]) => {
                                 // @ts-expect-error Error Unspecific API
                               ...
                               onChange = {(selected: { label: string; value: string }[]) => {
                                 setTableState((prev) => ({
                                   ...prev, columnVisibility: {
                                     // @ts-expect-error Error Unspecific API
                                     ...columns.reduce((acc, c) => ({...acc, [c.accessorKey]: false}), {}),
                                     ...selected.reduce((acc, c) => ({...acc, [c.value]: true}), {})
                                   }
                                 }));
                               }}>
          ...
          <div className = "flex p-1 gap-1.5 flex-wrap">
              ...
              <Select classNames = {{...selectStyles}}
                ...
                // @ts-expect-error Error Unspecific API
                      ...
                      value = {
                        ...
                        //   value: c.value
                        // }))}
                        (tableState.columnFilters.find((f) => f.id === filter)) ? ((tableState.columnFilters.find((f) => f.id === filter)?.value as string[]).map((s) => ({
                          label: s,
                          value: s
                        }))) : []}
                      name = {filter}
            ...
            {tableState.columnFilters.filter((f) => !permenentColumnFilters.includes(f.id)).map((filter) => (
              // @ts-expect-error Error already handled
              columns.find((c) => c.accessorKey === filter.id)?.meta.type === 'categorical' ?
                <Select classNames = {{...selectStyles}}
                  ...
                  // @ts-expect-error Error Unspecific API
                        ...
                        isClearable = {true}
                        isMulti = {true}
                        value = {(tableState.columnFilters.find((f) => f.id === filter.id)) ? ((tableState.columnFilters.find((f) => f.id === filter.id)?.value as string[]).map((s) => ({
                          label: s,
                          value: s
                        }))) : []}
                        name = {filter.id}
                  // @ts-expect-error Error already handled
                        ...
                        onChange = {handleFilterChange}
                        onMenuClose = {() => {
                          (tableState.columnFilters.find((f) => f.id === filter.id)?.value as string[]).length === 0 &&
                          setTableState((prev) => ({
                            ...prev,
                            columnFilters: prev.columnFilters.filter((f) => f.id !== filter.id)
                          }));
                        }}
                  // @ts-expect-error Error already handled
                /> : columns.find((c) => c.accessorKey === filter.id)?.meta.type === 'text' ?
                  ...
                  >
                    ...
                    <AutosizeInput
                      ...
                      onChange = {(e: { target: { value: string; }; }) => {
                        setTableState((prev) => ({
                          ...prev,
                          columnFilters: prev.columnFilters.map((f) => f.id === filter.id ? {
                            ...f,
                      ...
                      minWidth = "30"
                      value = {tableState.columnFilters.find((f) => f.id === filter.id)?.value as string || ''}
                    />
                    <button
                      onClick = {() => {
                        setTableState((prev) => ({
                  ...
                  >
                    ...
                    <DatePicker
                      ...
                      selectsRange
                      // @ts-expect-error Error already handled
                      startDate = {tableState.columnFilters.find((f) => f.id === filter.id)?.value[0]}
                      // @ts-expect-error Error already handled
                      endDate = {tableState.columnFilters.find((f) => f.id === filter.id)?.value[1]}
                      ...
                      onBlur = {() => {
                        // @ts-expect-error Error already handled
                        tableState.columnFilters.find((f) => f.id === filter.id)?.value.length === 0 &&
                        setTableState((prev) => ({
                          ...prev,
                          columnFilters: prev.columnFilters.filter((f) => f.id !== filter.id)
                        }));
            ...
            <Select classNames = {{...selectStyles}}
              ...
              // @ts-expect-error Error already handled
                    options = {columns.filter((c) => !tableState.columnFilters.find((f) => f.id === c.accessorKey) && !permenentColumnFilters.includes(c.accessorKey)).map((c) => ({
                      label: c.header,
                      // @ts-expect-error Error already handled
                      value: c.accessorKey
                    }))}
                    onChange = {(newValue) => {
                      setTableState((prev) => ({
                        ...
                        columnFilters: [...prev.columnFilters, {
                          ...
                          // @ts-expect-error Error already handled
                          value: columns.find((c) => c.accessorKey === newValue?.value)?.meta?.type === 'categorical' || columns.find((c) => c.accessorKey === newValue?.value)?.meta?.type === 'daterange' ? [] : ""
                        }]
                      }))
            ...
            {tableState.columnFilters.length > 0 &&
                <button className = "text-sm" onClick = {() => setTableState((prev) => ({...prev, columnFilters: []}))}>Clear all</button>}
          </div>
